...
#include "ui_videoreplaycontrolusb.h"
...
VideoreplayControlUsb::VideoreplayControlUsb(QWidget *parent)
    : QWidget(parent)
    , ui(new Ui::VideoreplayControlUsb)
{
    ui->setupUi(this);
    ...
    ledCamera1 = new LEDWidget(5553);
    ledCamera2 = new LEDWidget(5563);
    ledCamera3 = new LEDWidget(5573);

    ui->hbTurnCameras->addWidget(ledCamera1);
    ui->hbTurnCameras->addWidget(new QLabel("Камера 1"));
    ui->hbTurnCameras->addWidget(ledCamera2);
    ui->hbTurnCameras->addWidget(new QLabel("Камера 2"));
    ui->hbTurnCameras->addWidget(ledCamera3);
    ui->hbTurnCameras->addWidget(new QLabel("Камера 3"));
    ui->hbTurnCameras->setAlignment(ledCamera1, Qt::AlignVCenter);
    ui->hbTurnCameras->setAlignment(ledCamera2, Qt::AlignVCenter);
    ui->hbTurnCameras->setAlignment(ledCamera3, Qt::AlignVCenter);
    ui->hbTurnCameras->addStretch();

    connect(ui->cbTurnCameras, &QCheckBox::checkStateChanged, this, &VideoreplayControlUsb::turnCameras);

    procAdbFind = new QProcess(this);
    procAdbFind->setProgram("adb/adb.exe");

    procAdbConnCam = new QProcess(this);
    //procAdbConnCam->setProgram("adb/adb.exe");

    argsCam1<<"-fflags"<<"nobuffer"<<"-thread_queue_size"<<"512"<<"-use_wallclock_as_timestamps"<<"1"
            <<"-i"<<"tcp://127.0.0.1:5551?listen"<<"-f"<<"s16le"<<"-thread_queue_size"<<"512"
            <<"-i tcp://127.0.0.1:5552";

    argsCam2<<"-fflags"<<"nobuffer"<<"-thread_queue_size"<<"512"<<"-use_wallclock_as_timestamps"<<"1"
             <<"-i"<<"tcp://127.0.0.1:5561?listen"<<"-f"<<"s16le"<<"-thread_queue_size"<<"512"
             <<"-i tcp://127.0.0.1:5562";

    argsCam3<<"-fflags"<<"nobuffer"<<"-thread_queue_size"<<"512"<<"-use_wallclock_as_timestamps"<<"1"
             <<"-i"<<"tcp://127.0.0.1:5571?listen"<<"-f"<<"s16le"<<"-thread_queue_size"<<"512"
             <<"-i tcp://127.0.0.1:5572";

    connect(procAdbFind, &QProcess::readyReadStandardOutput, this, &VideoreplayControlUsb::adbFind);
    ...
    //         connectionSocket->connectToHost("127.0.0.1", 5553);
    //         connectionSocket->waitForConnected(1000);
    //         connectionSocket->write("hello");
    //         connectionSocket->waitForReadyRead(1000);
    //         QByteArray ba = connectionSocket->readAll();
    //         if(ba.size() > 0){
    //             int h = ba.at(0);
    //             qDebug()<<"connected camera "<<h;
    //         }
    //         connectionSocket->disconnectFromHost();
    //     }
    //     if(connectedCameras[2] != ""){
    //         connectionSocket->connectToHost("127.0.0.1", 5563);
    //         connectionSocket->waitForConnected(1000);
    //         connectionSocket->write("hello");
    //         connectionSocket->waitForReadyRead(1000);
    //         QByteArray ba = connectionSocket->readAll();
    ...
    //         connectionSocket->disconnectFromHost();
    //     }
    //     if(connectedCameras[3] != ""){
    //         connectionSocket->connectToHost("127.0.0.1", 5573);
    //         connectionSocket->waitForConnected(1000);
    //         connectionSocket->write("hello");
    //         connectionSocket->waitForReadyRead(1000);
    //         QByteArray ba = connectionSocket->readAll();
    ...
    // connect(videoSettings, &SettingsVideoReplay::sigShowReplayOnTv, this, &VideoreplayControlUsb::sigShowReplayOnTv);

}

VideoreplayControlUsb::~VideoreplayControlUsb()
{
    killFfmpegProcess();
    qDebug()<<"dest";
    delete player;
...
void VideoreplayControlUsb::setPlayerTv(PlayerTv *p)
{
    player->setTvPlayer(p);
}

void VideoreplayControlUsb::killFfmpegProcess()
{
    // ui->cbCam1->setChecked(false);
    // ui->cbCam2->setChecked(false);
    // ui->cbCam3->setChecked(false);
...
void VideoreplayControlUsb::offStreamVk()
{
...
void VideoreplayControlUsb::getHWcodec()
{
    QProcess proc(this);
...
void VideoreplayControlUsb::startReadCams()
{
    procReadCam1->start();
    procReadCam2->start();
    procReadCam3->start();
...
void VideoreplayControlUsb::stopReadCams()
{
    qDebug()<<"stopReadCams";
    procRead->write("q");
...
void VideoreplayControlUsb::turnCameras(bool state)
{
    if(state){
        procAdbFind->setArguments({"devices"});
        procAdbFind->start();
        ledCamera1->start();
        ledCamera2->start();
        ledCamera3->start();
        ...
        QProcess proc;
        proc.setProgram("adb/adb.exe");
        proc.setArguments({"kill-server"});
        proc.start();
...
void VideoreplayControlUsb::onStreamVk()
{
    QStringList args;
...
void VideoreplayControlUsb::adbFind(void)
{
    ...
    connectedCameras[2] = "";
    connectedCameras[3] = "";
    qDebug()<<connectedCameras;
    while(true){
        char buf[1024];
        qint64 len = procAdbFind->readLine(buf, sizeof(buf));
        if(len == -1) break;
        if(len == 0) break;
        sList<<buf;
    }
    ...
    foreach(QString s, sList){
        if(s.contains("\tdevice")){
            listDevices<<s.split("\t").at(0);
        }
    }
    qDebug()<<"sList = "<<listDevices;
    foreach(auto device, listDevices){
        QStringList args;
        //procAdbFind.setProgram("adb/adb.exe");
        procAdbConnCam->setProgram("adb/adb.exe");
        args<<"-s"<<device<<"forward"<<"tcp:6666"<<"tcp:6666";
        procAdbConnCam->setArguments(args);
        procAdbConnCam->start();
        procAdbConnCam->waitForFinished(1000);
        procAdbConnCam->waitForReadyRead(1000);
        qDebug()<<procAdbConnCam->readAllStandardOutput()<<device<<procAdbConnCam->arguments();
        procAdbConnCam->close();
        QTcpSocket socket;
    ...
    QStringList args;
    if(connectedCameras[1] != ""){
        args<<"-s"<<connectedCameras[1]<<"forward"<<"tcp:5553"<<"tcp:5553";
        procAdbConnCam->setArguments(args);
        procAdbConnCam->start();
    }
    if(connectedCameras[2] != ""){
        procAdbConnCam->waitForFinished();
        args.clear();
        args<<"-s"<<connectedCameras[2]<<"forward"<<"tcp:5563"<<"tcp:5563";
        procAdbConnCam->setArguments(args);
        procAdbConnCam->start();
    }
    if(connectedCameras[3] != ""){
        procAdbConnCam->waitForFinished();
        args.clear();
        args<<"-s"<<connectedCameras[3]<<"forward"<<"tcp:5573"<<"tcp:5573";
        procAdbConnCam->setArguments(args);
        procAdbConnCam->start();
    }
...
void VideoreplayControlUsb::setWebCam1(QString cam)
{
    urlWebCam1 = cam;
}

void VideoreplayControlUsb::setWebCam2(QString cam)
{
    urlWebCam2 = cam;
}

void VideoreplayControlUsb::setParamWebCam1(QList<int> param)
{
    fps1 = QString::number(param.at(0));
    resolution1 = QString::number(param.at(1)) + "x" + QString::number(param.at(2));
}

void VideoreplayControlUsb::setParamWebCam2(QList<int> param)
{
    fps2 = QString::number(param.at(0));
    resolution2 = QString::number(param.at(1)) + "x" + QString::number(param.at(2));
}

void VideoreplayControlUsb::setSound(QString sound)
{
    urlSound = sound;
}

void VideoreplayControlUsb::setCam1(QString cam)
{
    urlCam1 = cam;
    qDebug()<<"urlCam1 = "<<urlCam1;
}

void VideoreplayControlUsb::setCam2(QString cam)
{
    urlCam2 = cam;
    qDebug()<<"urlCam2 = "<<urlCam2;
}

void VideoreplayControlUsb::setCam3(QString cam)
{
    urlCam3 = cam;
    qDebug()<<"urlCam3 = "<<urlCam3;
}

void VideoreplayControlUsb::startRecord(bool b, QString s)
{
...
void VideoreplayControlUsb::stopRecord(bool b)
{
    btnPlay->setEnabled(true);
    btnPlayLast->setEnabled(true);
...
// void VideoreplayControlUsb::statesCameras(int state){
